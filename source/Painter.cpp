...
bool Painter::initialize()
{
	...
	glBindTexture(GL_TEXTURE_2D, m_envmaps[PolarMapping]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
	glBindTexture(GL_TEXTURE_2D, 0);
/*	glTexParameterf(m_envmaps[PolarMapping], GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);
	glTexParameterf(m_envmaps[SphereMapping], GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
	glTexParameterf(m_envmaps[ParaboloidMapping], GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
*/
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeDepthRB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, &m_cubeTex);
    ...
    foreach(const int i, m_programs.keys())
    {
        ...
        if (programs.contains(program) && program->isLinked())
        {
            ...
            case EnvMapProgram:
                ...
                program->setUniformValue("viewProjectionInv", camera()->viewProjectionInverted());
            ...
            case SphereProgram:
                ...
                program->setUniformValue("transform", camera()->viewProjection() * m_transforms[1]);
                program->setUniformValue("eye", camera()->eye());
				program->setUniformValue("ico_center", m_icosa_center);
                // ...?
